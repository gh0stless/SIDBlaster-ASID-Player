...
MainComponent::MainComponent()
{
    ...
    outputTextBox.insertTextAtCaret("SIDBlaster ASID Protocol Player 0.6.3 (beta)\n");
                ...
                for (uint8_t bit = 0; bit < 7; bit++) {  /* each packet has 7 bits ~ stoopid midi */
                    if (data[mask + 2] & (1 << bit)) {  /* 3 byte message, skip 3 each round and add the bit */
                        ...
                        if (address <= NUMSIDREGS) {
                            ...
                            outputTextBox.insertTextAtCaret("incorrect data resived\n");
                        }
                        reg++;
                    }
                }
...
void MainComponent::timerCallback()
{
    ...
    {
        if (Msg2Mem && (sid->Number_Of_Devices > 1)) {
    ...
    {
        if (Msg3Mem && (sid->Number_Of_Devices > 2)) {
